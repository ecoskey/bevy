...
pub fn init_auto_exposure_pipeline(
    ...
    let shader = load_embedded_asset!(asset_server.as_ref(), "auto_exposure.wgsl");
    ...
    let variants = SpecializedCache::new(AutoExposureSpecializer, base_descriptor);

    commands.insert_resource(AutoExposurePipeline { layout, variants });
}
