...
pub trait IterSystem: System + Sized {
    ...
    fn run_iter_without_applying_deferred<'a, I: IntoIterator<Item = SystemIn<'a, Self>> + 'a>(
        ...
        match valid {
            // SAFETY:
            // - We have exclusive access to the entire world.
            Ok(()) => SystemIter::Valid(unsafe { self.run_iter_unsafe(input, world_cell) }),
            Err(err) => SystemIter::Invalid(Some(err)),
        }
    }

    fn run_iter_readonly<'a, I: IntoIterator<Item = SystemIn<'a, Self>> + 'a>(
        &'a mut self,
        input: I,
        world: &'a World,
    ) -> SystemIter<'a, Self, I::IntoIter>
...
/// A wrapper around raw system iterators that supports validation.
///
/// This iterator short-circuits if the system is invalid, yielding
/// a single `SystemParamValidationError` wrapped into a `Result<S::Out, RunSystemError>`
pub enum SystemIter<'a, S: IterSystem, I: Iterator<Item = SystemIn<'a, S>> + 'a> {
    Invalid(Option<SystemParamValidationError>),
    Valid(S::Iter<'a, I>),
}

impl<'a, S: IterSystem, I: Iterator<Item = SystemIn<'a, S>> + 'a> Iterator
    for SystemIter<'a, S, I>
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        match self {
            SystemIter::Invalid(err) => err.take().map(|err| Err(err.into())),
            SystemIter::Valid(iter) => iter.next(),
        }
    }
}
